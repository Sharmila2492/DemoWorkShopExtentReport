package Utils;

import java.io.File;
import java.io.FileReader;
import java.io.IOException;
import java.time.Duration;
import java.util.Properties;

import org.apache.commons.io.FileUtils;
import org.apache.poi.xssf.usermodel.XSSFCell;
import org.apache.poi.xssf.usermodel.XSSFRow;
import org.apache.poi.xssf.usermodel.XSSFSheet;
import org.apache.poi.xssf.usermodel.XSSFWorkbook;
import org.openqa.selenium.OutputType;
import org.openqa.selenium.TakesScreenshot;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.edge.EdgeDriver;

import com.aventstack.extentreports.ExtentReports;
import com.aventstack.extentreports.ExtentTest;

public class Utility {
	
public static  WebDriver driver1;   //=new ChromeDriver();// remove static keyword for parallel execution
	public static String filepath;
	public String SheetName;
	public static Properties prop;
	public static String filepath1;
	public static ExtentReports extent;
	public static ExtentTest test;
	public String testName, testDescription, testCategory, testAuthor;
	
	//public void Browser()
	public void Browser(String url, String browser)// it is used for cross parallel test
	{
	    if(browser.equalsIgnoreCase("chrome"))
	    {
		   driver1=new ChromeDriver();	
	    }
	    else if(browser.equalsIgnoreCase("Edge"))
	    {
	    	driver1=new EdgeDriver();		
	    }
	    else
	    {
	    	driver1=new ChromeDriver();	
	    }
		//driver1.get("https://demowebshop.tricentis.com/");
		driver1.get(url);
		driver1.manage().window().maximize();
		driver1.manage().timeouts().implicitlyWait(Duration.ofSeconds(20));
	}
	
	public void closeBrowser()
	{
		driver1.close();
	}
	
	public static String[][] readExcel(String sheetName) throws IOException
	{
		XSSFWorkbook book =new XSSFWorkbook("D:\\JavaWorkspace\\TestNG.DemoWorkShop.ExtentReport\\src\\test\\resources\\data\\Register.xlsx");
		XSSFSheet Sheet=book.getSheet(sheetName);
		int rowcount = Sheet.getLastRowNum();
		short columncount = Sheet.getRow(0).getLastCellNum();
		
		String[][] data= new String[rowcount][columncount];
		for(int i=1 ; i<=rowcount; i++)
		{
			XSSFRow row =Sheet.getRow(i);
			
			for(int j=0; j<columncount; j++)
			{
				XSSFCell cell = row.getCell(j);
				System.out.println(cell.getStringCellValue());
				data[i-1][j]= cell.getStringCellValue();
				
			}
			
		
		}
		
		book.close();
		return data;
		}
				
		
		
	public static void readFromPropFile(String filepath1) throws IOException
   {
	   FileReader file=new FileReader(filepath1); //read the data from the file
       prop=new Properties();  //maintain all the properties in the particular file
     prop.load(file); //load all the data into the file
     
   }
	
	public static String screenshot(String name) throws IOException
	{
		String path="D:\\JavaWorkspace\\TestNG.DemoWorkShop.ExtentReport\\Screenshot\\"+name+".png";
	    File src=((TakesScreenshot)driver1).getScreenshotAs(OutputType.FILE);
	    File dest=new File(path);
	    FileUtils.copyFile(src, dest);
	    return path;
	}
}


